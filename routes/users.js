...
router.post('/login',  async (req, res) => {
    let sql = 'select * from jot_users where email= $1';
    let sql_value = [req.body.email];
    client.query(sql, sql_value, async (err, results) => {
        if (err) throw err.message;
        // console.log(results.rows[0].id);
        console.log(results.rows); 
        if(results.rows == 0){
            req.flash('error_msg', `Email is Incorrect`);
            res.render('login', {
                msg: 'Email is Incorrect',
                email: req.body.email
            });
        }else{
            const validPasswd = await bcrypt.compare(req.body.passwords, results.rows[0].passwords);
            console.log(validPasswd);

            if (validPasswd === false) {
                req.flash('error_msg', `Passwords is Incorrect`);
                res.render('login',{
                    msg: 'Passwords is Incorrect',
                    email: req.body.email,
                });
            } else {
                ...
                // client.query('select * from jot_users where email = $1', [req.body.email], (err, results) => {
                //     if(err) throw err;
